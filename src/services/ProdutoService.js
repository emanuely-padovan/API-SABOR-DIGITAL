...
class ProdutoService {
    async listarProdutos() {
        const produtos = await ProdutoRepository.encontrarProdutos()
        return {
            sucesso: true,
            dados: produtos,
    ...
    async buscarProdutoporId(id) {
        ...
        const produto = await ProdutoRepository.encontrarProdutosPorId(id)
        if (!produto) {
            throw {
                status: 404,
    ...
    async atualizarProduto(id, dados) {
        ...
        const existe = await ProdutoRepository.encontrarProdutosPorId(id)
        if (!existe) {
            throw {
                status: 404,
                mensagem: "Produto não encontrado!"
        ...
        await ProdutoRepository.atualizar(id, atualizado)
